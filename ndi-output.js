...
function criarProcessoNDI(nomeFonte, comAudio = false, audioUrl = null) {
    ...
    if (comAudio) {
        ...
        if (arquivoAudio && fs.existsSync(arquivoAudio)) {
            ...
            args.push('-thread_queue_size', '1024', '-i', arquivoAudio);
        } else {
            args.push('-f', 'lavfi', '-i', 'anullsrc=channel_layout=stereo:sample_rate=48000');
        }
        args.push('-map', '0:v:0', '-map', '1:a:0?', '-c:a', 'pcm_s16le', '-ar', '48000', '-ac', '2', '-af', 'aresample=async=1:first_pts=0');
    }

    args.push('-f', 'libndi_newtek', nomeFonte);

    const proc = spawn(ffmpegPath, args, { stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true });

    proc.stdin.on('error', err => console.log(`⚠️ ${nomeFonte}: stdin fechado (${err.code})`));
    proc.stderr.on('data', data => { const msg = data.toString(); if (!msg.includes('frame=')) console.log(`[NDI ${nomeFonte}]: ${msg}`); });
    proc.on('close', code => console.log(`NDI ${nomeFonte} encerrado. Código: ${code}`));
...
function reiniciarExibidor(audioUrl) {
    ...
    ffmpegExibidor = criarProcessoNDI('ZapMix - Exibidor de Mensagens', true, audioUrl);
    ultimoAudioUrl = audioUrl || null;
    console.log(audioUrl ? `🔊 Áudio Exibidor ativo: ${audioUrl}` : '🔇 Exibidor sem áudio');
}

function processoValido(proc) {
    return proc && !proc.killed && proc.exitCode === null && proc.stdin && !proc.stdin.destroyed && proc.stdin.writable;
}

async function capturarFrame(win) {
    if (!win || win.isDestroyed()) return null;
    const image = await win.webContents.capturePage();
    return image.resize({ width: NDI_WIDTH, height: NDI_HEIGHT }).toBitmap();
}

function escreverFrame(proc, buffer) {
    if (!processoValido(proc) || !buffer) return;
    try { proc.stdin.write(buffer); } catch (err) { console.log('Erro ao escrever frame:', err.message); }
}
...
function iniciarNDI({ exibidorWindow, enqueteWindow, porta }) {
    ...
    ffmpegExibidor = criarProcessoNDI('ZapMix - Exibidor de Mensagens', true, null);
    ...
    interval = setInterval(async () => {
        ...
        try {
            ...
            if (contadorEstado >= FPS) {
                ...
                if (audioAtual !== ultimoAudioUrl) reiniciarExibidor(audioAtual);
            }
        ...
        } catch (err) { console.log('Erro no NDI:', err.message); }
    }, Math.round(1000 / FPS));
...
module.exports = { iniciarNDI, pararNDI };